...
import static com.qg.zhongjianyu.util.Base64UrlUtil.base64UrlDecode;
import static com.qg.zhongjianyu.util.Base64UrlUtil.base64UrlEncode;

/**
 * JWT生成器
 * Token内部有三个部分：Header、Payload、Signature
 * Payload部分有四个字段：exp(过期时间)、userId(用户id)、userName(用户名)、status(用户状态/用户权限)
 * @author 钟健裕
 */
public class Token {
    ...
    static {
        ...
        try {
            properties.load(Token.class.getResourceAsStream("/MyToken.properties"));
    ...
    public static Token getToken(String access_token) throws JsonProcessingException {
        ...
        //判断是否要刷新token
        long expTime = Long.parseLong((String) tokenPackage.payload.get("exp"));
        if (System.currentTimeMillis() - REFRESH_TIME > expTime){
            //刷新token
            tokenPackage.tokenCode = Code.REFRESH;
            //生成新的token
    ...
    public boolean verify(){
        ...
        if (!signTure.equals(parts[2])) {
            ...
            // 验证过期时间
            long expTime = Long.parseLong((String) payload.get("exp"));

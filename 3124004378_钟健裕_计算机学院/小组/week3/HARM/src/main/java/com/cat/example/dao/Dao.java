...
import static com.cat.example.dao.ORM.ResultSetToObject.resultSetToObject;
import static com.cat.example.dao.ORM.SQLGenerator.*;
...
public class Dao {
    ...
    public <T> T selectClass(Class<?> entityClass,String where) {
        ...
        //将结果集转换为对象
        try {
            return (T) resultSetToObject(rs, entityClass);
    ...
    public <T> List<T> selectList(Class<T> entityClass, String where) {
        ...
        try (ResultSet rs = select(entityClass, where)) {
            ...
            while (rs.next()) {
                //将结果集的每一行转换为对象
                T obj = (T) resultSetToObject(rs, entityClass);
                list.add(obj);
            }
            return list;
        } catch (SQLException | InvocationTargetException | IllegalAccessException | InstantiationException |

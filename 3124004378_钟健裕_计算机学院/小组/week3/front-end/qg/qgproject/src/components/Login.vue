			...
			<div class="login-form">
				<el-form :model="loginForm" :rules="loginRules" ref="loginFormRef">
					<el-form-item prop="phone">
						<el-input icon="User" v-model="loginForm.phone" placeholder="请输入手机号" />
					</el-form-item>

					<el-form-item prop="password">
						<el-input type="password" icon="Lock" v-model="loginForm.password" placeholder="请输入密码" />
					</el-form-item>
...
async function loginRequest(loginForm) {
	...
	//发送axios请求
	try {
		return await axios.post('http://localhost:8080/zhongjianyu.qg.com/api/user/login', loginData);
	} catch (error) {
		console.log(error);
		ElNotification({
			title: '登录失败',

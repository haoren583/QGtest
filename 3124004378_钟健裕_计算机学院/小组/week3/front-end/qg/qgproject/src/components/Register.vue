            ...
            <div class="register-form">
                <el-form :model="registerForm" :rules="registerRules" ref="registerFormRef">
                    <el-form-item prop="useremail">
                        <el-input prefix-icon="User" v-model="registerForm.phone" placeholder="请输入手机号" />
                    </el-form-item>
                    <el-form-item prop="name">
                        <el-input prefix-icon="User" v-model="registerForm.userName" placeholder="请输入姓名" />
                    </el-form-item>
                    ...
                    <el-form-item style="margin: 50px 0;">
                        <el-button :disabled="canClick" type="danger" @click="RegisterSubmit(registerFormRef)"
                            :loading="isLoading" style="width: 100%;">注册</el-button>
                    </el-form-item>
...
const canClick = computed(() => {
    if (registerForm.phone !== '' && registerForm.password !== '' && registerForm.confirmPassword !== ''&& registerForm.password === registerForm.confirmPassword&& registerForm.name !== '') { return false }
    else { return true }
})
//表单校验规则
...
async function registerRequest(registerForm) {
    ...
    //发送axios请求
    try {
        return await axios.post('http://localhost:8080/zhongjianyu.qg.com/api/user/register', registerDataJson);
    } catch (error) {
        console.log(error);
        ElNotification({
            title: '注册失败',
